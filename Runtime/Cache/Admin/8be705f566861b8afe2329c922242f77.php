        ...
        <div class="panel dux-box  active">
            <div class="panel-head">
                <div class="tab-head">
                    <strong>站点信息</strong>
                    <ul class="tab-nav">
                        <li class="active"><a href="#tab-1">上传设置</a></li>
                    </ul>
                </div>
            </div>
            <div class="tab-body">
                <div id="tab-1" class="tab-panel active">
                    <div class="form-group">
                        <div class="label"><label>上传大小：</label></div>
                        <div class="field">
                        <input type="text" name="maxSize" value="<?php echo ($config["maxSize"]); ?>" class="input input_small" size="60" />
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="label"><label>保存路径：</label></div>
                        <div class="field">
                        <input type="text" name="savePath" value="<?php echo ($config["savePath"]); ?>" class="input input_big" size="60" />
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="label"><label>文件后缀：</label></div>
                        <div class="field">
                        <input type="text" name="allowExts" value="<?php echo ($config["allowExts"]); ?>" class="input input_big" size="60" />
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="label"><label>文件类型：</label></div>
                        <input type="text" name="allowTypes" value="<?php echo ($config["allowTypes"]); ?>" class="input input_big" size="60" />
                    </div>
                    <div class="form-group">
                        <div class="label"><label>缩略图：</label></div>
                        <div class="field button-group button-group-small radio">
                        <?php if($config['thumb'] == 1): ?><label class="button active"><input type="radio" value="1" name="thumb" checked="checked">
                        <?php else: ?>
                            <label class="button"><input type="radio" value="1" name="thumb"><?php endif; ?>
                            <span class="icon icon-check"></span> 启用</label>
                        <?php if($config['thumb'] == 0): ?><label class="button active"><input type="radio" value="0" name="thumb" checked="checked">
                        <?php else: ?>
                            <label class="button"><input type="radio" value="0" name="thumb"><?php endif; ?>
                            <span class="icon icon-times"></span> 禁用</label>
                        </div>
                    </div>

                    <div class="form-group">
                        <div class="label"><label>缩略图宽度：</label></div>
                        <div class="field">
                        <input type="text" name="thumbMaxWidth" value="<?php echo ($config["thumbMaxWidth"]); ?>" class="input input_big" size="60" />
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="label"><label>缩略图高度：</label></div>
                        <div class="field">
                        <input type="text" name="thumbMaxHeight" value="<?php echo ($config["thumbMaxHeight"]); ?>" class="input input_big" size="60" />
                        </div>
                    </div>                 
                    <div class="form-group">
                        <div class="label"><label>缩略图前缀：</label></div>
                        <div class="field">
                        <input type="text" name="thumbPrefix" value="<?php echo ($config["thumbPrefix"]); ?>" class="input input_big" size="60" />
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="label"><label>缩略图保存路径：</label></div>
                        <div class="field">
                        <input type="text" name="thumbPath" value="<?php echo ($config["thumbPath"]); ?>" class="input input_big" size="60" />
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="label"><label>是否删除原图：</label></div>
                        <div class="field button-group button-group-small radio">
                        <?php if($config['thumbRemoveOrigin'] == 1): ?><label class="button active"><input type="radio" value="1" name="thumbRemoveOrigin" checked="checked">
                        <?php else: ?>
                            <label class="button"><input type="radio" value="1" name="thumbRemoveOrigin"><?php endif; ?>
                            <span class="icon icon-check"></span> 启用</label>
                        <?php if($config['thumbRemoveOrigin'] == 0): ?><label class="button active"><input type="radio" value="0" name="thumbRemoveOrigin" checked="checked">
                        <?php else: ?>
                            <label class="button"><input type="radio" value="0" name="thumbRemoveOrigin"><?php endif; ?>
                            <span class="icon icon-times"></span> 禁用</label>
                        </div>
                    </div>
                </div>
