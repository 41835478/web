...
function get_meta_value(){
    ...
    $data= M('router')->field('sjurl,lyurl')->select();
    $array = array();
    if($data){
        foreach ($data as $value) {
            $array[$value['lyurl']] = $value['sjurl'];
        }
    }
...
function send_mail($receiver, $subject, $body, $attachment){
    return R('Addons://Email/Email/sendMail', array($receiver));
}
 ...
 * @author jry <598821125@qq.com>
 */
function send_tel($receiver){
    return R('Addons://TelMsg/TelMsg/sendTel', array($receiver));
}
