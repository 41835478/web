...
class Page{
    ...
    public $isRouter = 0; // 是否启用路由规则
    public $lastSuffix = true; // 最后一页是否显示总页数

    private $p       = 'p'; //分页参数名
    private $url     = '/h/'; //当前链接URL
    private $nowPage = 1;
    ...
    public function show() {
        ...
        if($this->isRouter == 1){
            $this->url = U("/h/".$this->parameter["tid"].'-'.$this->parameter["yid"].'-'.urlencode($this->parameter["p"]));
        }elseif ($this->isRouter == 2) {
            $this->url = U("/d/".$this->parameter["tid"].'-'.$this->parameter["yid"].'-'.urlencode($this->parameter["p"]));
        }else{
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page > 0 && $this->totalPages != 1){
                    $link_page .= '<li class="current am-active"><a>' . $page . '</a></li>';
                }
            }
        }
